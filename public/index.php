...
$router->map('GET', '/photos/[i:id]', function ($id) use ($client, $redis) {
    if (!$redis->exists('photo:' . $id)) {
        $response = $client->request('GET', 'https://jsonplaceholder.typicode.com/photos/' . $id);
        $redis->setex('photo:' . $id, REDIS_STANDARD_EXPIRY, $response->getBody()->getContents());
    }

    header('Content-Type: application/json; charset=utf-8');

    echo json_encode([
        'data' => json_decode($redis->get('photo:' . $id))
    ]);
});
